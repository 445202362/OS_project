...
public class PriorityScheduler {
	...
	public PriorityScheduler(LinkedList<PCB> readyQueue, int totalProcesses, MemoryManager memoryManager) {
		...
		while (finishedProcesses.size() < totalProcesses) {
			...
			if (runningProcess == null) {
				...
				if (runningProcess != null) {
					...
					System.out.println("[t=" + currentTime + "] Dispatched P" + runningProcess.getId() + " | priority="
							+ runningProcess.getPriority() + " | remaining burst=" + burstAtStart + "ms");
				}
			}
		...
		synchronized (readyQueue) {
			...
			for (int i = 0; i < readyQueue.size(); i++) {
				...
				if (p.getTimeInReadyQueue() > starvationThreshold) {
					if (!p.isStarved()) {
						p.setStarved(true);
						System.out.println("[t=" + currentTime + "] STARVATION detected: P" + p.getId() + " | waited="
								+ p.getTimeInReadyQueue() + "ms | threshold=" + starvationThreshold + "ms");
					}
					int oldPriority = p.getPriority();
					p.applyAging();
					if (p.getPriority() < oldPriority) {
						System.out.println("[t=" + currentTime + "] Aging applied to P" + p.getId() + " | new priority="
								+ p.getPriority());
					}
				}
			}
